...
@section('content')
    ...
    <div class="container-xxl py-2 border fw-bolder" style="height: auto;text-align:right;direction: rtl"> 
        ...
        /
        @if ($current_employee->institution->main_institution)
        <a href="{{route('showInstitution',$current_employee->institution->main_institution->institution_slug)}} ">{{$current_employee->institution->main_institution->institution_name}}</a> 
        /
        @endif
        <a href="{{route('showInstitution',$current_employee->institution->institution_slug)}}">{{$current_employee->institution->institution_name}}</a>  
        / 
        <a href="{{route('showEmployee',$current_employee->employee_slug)}}">{{$current_employee->employee_full_name}}</a>  
        / 
        <a class="text-dark"> مناقلة من عهدة</a> 
    </div> 
    <!-- root page End -->
                        ...
                        <div class="position-relative text-center mb-1 pb-2 wow fadeInUp" data-wow-delay="0.1s">
                            <div class="row">
                                ...
                                <div class="col-4">
                                    ...
                                    <p class="mb-1"> من عهدة موظف </p>
                                    <div class="form-floating form-group mb-1">
                                        <input  type="text" class="form-control @error('device_from_side_by_employee') is-invalid @enderror text-center" name="device_from_side_by_employee" id="device_from_side_by_employee" value="{{$current_employee->employee_full_name}}"  placeholder="الموظف" readonly>
                                        <input  type="hidden"  name="device_from_side_employee_slug" value="{{$current_employee->employee_slug}}" >
                        ...
                        <div class="container-xxl pt-5 pb-3" style="min-height: calc(100vh - 95px);direction: rtl">
                            <div class="container px-lg-5">
                                <div class="text-center mb-1 pb-1 wow fadeInUp" >
                                    <h2 class="mt-2">إختيارالطرف الثاني </h2>
                                ...
                                <div class="row g-4 mb-4 portfolio-container">
                                    ...
                                    @if ( $institution->employees->where('employee_slug' ,'<>',$current_employee->employee_slug)->count() >0)
                                                ...
                                                <div class="form-floating form-group mb-1">
                                                    @if ($institution->institution_kind == 'circle_pri')
                                                        <select class="form-control text-center @error('redirect_to_employee_side') is-invalid @enderror mb-2"  name="redirect_to_employee_side[]" id="redirect_to_employee_side" >
                                                            <option></option>
                                                            @forelse ($institution->employees->where('employee_slug' ,'<>',$current_employee->employee_slug) as $employee)
